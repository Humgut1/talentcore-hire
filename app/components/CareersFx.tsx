...
/* 첫 화면 뒤에 천천히 흐르는 어두운 강철빛 — 은빛 금속 로고가 서는 바탕.
...
export function Glow({ still }: { still: boolean }) {
  return (
    <MeshGradient
      ...
      colors={['#07090d', '#1a2130', '#07090d', '#2d3a52', '#0d1119']}
      distortion={0.8}
      swirl={0.4}
      grainMixer={0}
      grainOverlay={0.04}
...
/* =========================================================
   ...
   로고 바깥은 투명하다 — 뒤의 어두운 빛이 그대로 보인다.
   어두운 바탕이라 가장 어두운 띠(lo)를 조금 올리고 테두리 빛을 키웠다 —
   안 그러면 로고 윤곽이 바탕에 묻힌다.
...
const FS = `precision mediump float;uniform vec2 r;uniform float t;uniform sampler2D S;uniform sampler2D B;
...
export function MetalMark({ still, product = 'hire' }: { still: boolean; product?: Product }) {
