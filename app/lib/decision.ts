...
export const REJECT_REASONS: RejectReason[] = [
  ...
  { v: 'exp', side: 'us', l: '경력 수준 불일치', d: '역량은 있으나 연차·범위가 이 공고와 맞지 않음. 다른 공고로 연결할 수 있습니다.' },
...
export function rejectMailDraft(a: {
  ...
  if (r.side === 'them') {
    return {
      ...
      body:
        `${a.cand} 님, 안녕하세요.\n\n` +
        `${a.pos} 전형에 시간 내어 참여해 주셔서 감사합니다.\n` +
        `말씀해 주신 사정에 따라 이번 전형은 여기서 마무리하겠습니다.\n\n` +
        `함께 이야기 나눈 내용은 저희에게도 도움이 되었습니다. ` +
        `이후에 더 맞는 공고가 열리면 다시 연락드려도 괜찮을지 여쭙고 싶습니다.\n\n` +
        `좋은 결정 하시길 바랍니다.\n\n${a.sender} 드림`,
    }
  }
  ...
  if (a.code === 'position-closed') {
    return {
      ...
      body:
        `${a.cand} 님, 안녕하세요.\n\n` +
        `${a.pos} 전형에 참여해 주셔서 감사합니다.\n` +
        `내부 사정으로 해당 공고 채용이 중단되어, 부득이하게 전형을 종료하게 되었습니다.\n` +
