...
/* 데모 공고의 직무 설명.
   운영(Supabase)에 들어 있는 글과 같은 내용이라, 로컬에서 본 화면이
   실제 채용 사이트와 같은 분량·같은 구조로 보인다. */
const JD = {
  p1: `서버팀은 TalentCore 의 인사·근태·급여 데이터가 지나가는 길을 만듭니다. 고객사 한 곳의 잘못된 급여 계산이 그 회사 전 직원의 월급이 되는 자리라, '빨리 되는 것'보다 '틀리지 않는 것'을 먼저 봅니다.

[하는 일]
- 급여 마감·정산처럼 여러 표를 한 번에 바꾸는 작업의 트랜잭션 설계
- 근태 기록·결재 이벤트를 다루는 파이프라인 운영과 재처리 설계
- 고객사가 늘어도 응답 시간이 유지되도록 질의·색인·캐시 다듬기
- 장애가 났을 때 무엇이 언제 틀어졌는지 되짚을 수 있는 기록 남기기

[이런 분을 찾습니다]
- 서버 개발 5년 이상, 운영 중인 서비스를 직접 맡아 보신 분
- 관계형 데이터베이스의 트랜잭션·잠금·색인을 근거를 들어 설명하실 수 있는 분
- 비동기 처리나 메시지 큐로 시스템을 나눠 본 경험이 있으신 분
- 장애를 겪고 나서 원인과 대응을 글로 남겨 보신 분

[이런 분이면 더 좋습니다]
- 급여·회계처럼 돈이 걸린 계산을 다뤄 보신 분
- 여러 회사 데이터를 한 시스템에서 나눠 담는 구조를 설계해 보신 분
- Python 과 PostgreSQL 에 익숙하신 분

[함께 일하는 방식]
서버팀은 7명이고, 설계는 문서로 먼저 정리한 뒤 짧게 모여 정합니다. 배포는 하루에도 여러 번 하고, 만든 사람이 배포 후의 지표를 직접 봅니다. 장애 회고에서 사람을 찾지 않습니다 — 같은 일이 다시 안 일어나게 만드는 것까지가 회고입니다.`,

  p2: `디자인팀은 인사담당자가 하루 종일 보는 화면을 만듭니다. 처음 5분이 예쁜 화면보다, 8시간째에도 지치지 않는 화면을 만드는 일에 가깝습니다.

[하는 일]
- 채용·평가·급여처럼 단계가 긴 업무의 흐름을 처음부터 끝까지 설계
- 표와 목록, 결재선처럼 정보가 빽빽한 화면의 구조 잡기
- 디자인 시스템 관리 — 색은 상태에만 쓰고, 버튼과 강조는 무채색으로
- 실제 인사담당자와 함께 화면을 보며 막히는 지점 찾기

[이런 분을 찾습니다]
- 프로덕트 디자인 3년 이상, 출시한 제품의 화면을 포트폴리오로 보여 주실 수 있는 분
- 화면 하나가 아니라 여러 화면에 걸친 흐름을 설계해 보신 분
- 왜 그렇게 그렸는지를 취향이 아니라 근거로 설명하시는 분
- 개발자와 같은 언어로 이야기하는 데 거리낌이 없으신 분

[이런 분이면 더 좋습니다]
- B2B 나 업무용 소프트웨어를 다뤄 보신 분
- 디자인 시스템을 처음부터 세워 보신 분
- 간단한 화면은 직접 만들어 확인해 보시는 분

[함께 일하는 방식]
디자이너 4명이 제품 팀에 나뉘어 들어가 일합니다. 기획서를 받아 그리는 자리가 아니라, 무엇을 만들지부터 같이 정하는 자리입니다. 매주 한 번 서로의 작업을 놓고 이야기하고, 결정은 문서로 남깁니다.`,

  p3: `데이터팀은 인사 데이터를 '보고 판단할 수 있는 상태'로 만듭니다. 이직률·연차 사용률·부서 간 급여 차이 같은 숫자가 회사의 의사결정에 그대로 쓰이기 때문에, 숫자가 틀리면 결정이 틀립니다.

[하는 일]
- 인사·근태·급여 데이터를 모으는 파이프라인 설계와 운영
- 데이터 웨어하우스 모델링 — 분석하는 사람이 표를 뒤지지 않게
- 데이터 품질 점검 자동화 (빠진 값, 앞뒤가 안 맞는 값 잡아내기)
- 분석·리포트 화면이 쓰는 집계 표 만들기

[이런 분을 찾습니다]
- 데이터 엔지니어링 3년 이상
- SQL 을 읽고 쓰는 데 막힘이 없고, 느린 질의의 원인을 찾아 고쳐 보신 분
- Python 으로 파이프라인을 만들고 운영해 보신 분
- 데이터가 틀렸을 때 어디서부터 틀어졌는지 되짚어 보신 분

[이런 분이면 더 좋습니다]
- Airflow 등 작업 흐름 도구를 운영해 보신 분
- dbt 나 그에 준하는 방식으로 모델을 관리해 보신 분
- 개인정보가 섞인 데이터를 다뤄 보신 분

[함께 일하는 방식]
데이터팀은 5명이고, 주 2회 재택입니다. 분석가와 엔지니어가 같은 팀에 있어 '쓸 사람'과 '만들 사람'이 매일 붙어 있습니다. 새로 만드는 표는 누가 무엇에 쓰는지부터 적고 시작합니다.`,
}

export let positions: Position[] = [
  { id:'p1', title:'백엔드 엔지니어 (시니어)', dept:'플랫폼본부', team:'서버팀',   emp:'정규직', st:'open',   rec:'정수민', hm:'최영수', opened:'2026-07-20', ttf:23, band:[8000,10000], jd:JD.p1, loc:'서울 강남', exp:'경력 5년 이상', due:'2026-09-30' },
  { id:'p2', title:'프로덕트 디자이너',        dept:'프로덕트본부', team:'디자인팀', emp:'정규직', st:'open',   rec:'정수민', hm:'김서진', opened:'2026-07-28', ttf:15, band:[6500,8500],  jd:JD.p2, loc:'서울 강남', exp:'경력 3년 이상' },
  { id:'p3', title:'데이터 엔지니어',          dept:'플랫폼본부', team:'데이터팀', emp:'정규직', st:'open',   rec:'박현우', hm:'노아름', opened:'2026-06-30', ttf:43, band:[7500,9500],  jd:JD.p3, loc:'서울 강남 · 주 2회 재택', exp:'경력 3년 이상', due:'2026-09-20' },
